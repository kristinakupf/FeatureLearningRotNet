    ...
    def process_batch(self, batch, do_train=True):
        ...
        if isinstance(pred_var, (list, tuple)):
            ...
            for i in range(len(pred_var)):
                ...
                record['prec1_c'+str(1+i)] = accuracy(pred_var[i].data, labels, topk=(1,))
                record['prec5_c'+str(1+i)] = accuracy(pred_var[i].data, labels, topk=(5,))
        else:
            loss_total = self.criterions['loss'](pred_var, labels_var)
            record['prec1'] = accuracy(pred_var.data, labels, topk=(1,))
            record['prec5'] = accuracy(pred_var.data, labels, topk=(5,))
